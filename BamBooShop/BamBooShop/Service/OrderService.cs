    ...
    public class OrderService : IServiceBase<OrderDto, int>
    {
        ...
        public OrderDto Insert(OrderDto entity)
        {
            ...
            entity.OrderDetails.ForEach(x =>
            {
                ...
                //hmtien 28/8
                //if (x.Qty > product.Quantity)
                     //end when the qty is not available

                //hmtien add 25/8 
                product.Quantity = product.Quantity - x.Qty;
                products.Add(product);
            });
            
            //hmtien add 25/8
            products.ForEach(x => this.context.Products.Update(x));
        ...
        public bool InsertOrder(OrderDto entity)
        {
            DateTime dateNow = DateTime.Now;
            Order order = new Order()
            {
                CustomerCode = entity.CustomerCode,
                PhoneNumber = entity.Customer.PhoneNumber,
                Address = entity.Customer.Address+ ", "+ entity.Customer.District + ", "+ entity.Customer.City,
                Note = entity.Note,
                Status = Constants.OrderStatus.CHO_XAC_NHAN,
                Created = dateNow,
